...
// import * as AtlasAuth from './atlas-auth';
...
// function getProjectId() { return atlasConfig.get('project-id', ''); }

// function clusterUrl(projectId: string = getProjectId()) { 
//   return `https://cloud.mongodb.com/api/atlas/v1.0/groups/${projectId}/clusters`; 
// }
...
// const awsAmRegions = ['US_EAST_1', 'US_WEST_2', 'CA_CENTRAL_1', 'US_EAST_2', 'US_WEST_1', 'SA_EAST_1'];
// const awsAmRegionsShared = ['US_EAST_1', 'US_WEST_2', 'SA_EAST_1'];
...
// export function clusterSettings(instanceSizeName: string, providerName: string, regionName: string, clusterName: string): Object {
//   // Shared instances have providerName = TENANT
//   // and backingProviderName = providerName
...
// // I <3 FP
// export function validateClusterName(existingClusterNames: string[] = []): (clusterName: string) => string | undefined | null | Thenable<string|undefined|null> {
//   const invalidNameRegex = /[^A-z0-9\-]+/g;
//   return clusterName => clusterName.match(invalidNameRegex) || existingClusterNames.includes(clusterName) || clusterName.length === 0 ? 
//     'Cluster names can only contain ASCII letters, numbers, and hyphens, and must not be empty or duplicate existing cluster names' :
//     null;
// }

// export async function inputClusterName(existingClusterNames: string[] = []): Promise<string> {
//   return await vscode.window.showInputBox({
//     prompt: "Input cluster name", 
//     validateInput: validateClusterName(existingClusterNames)
//   }).then((result) => {
//     if (result) { return result; } 
//     else { throw new Error('No cluster name inputted'); }
//   });
// }

// export async function postCluster(instanceSizeName: string, providerName: string, regionName: string, clusterName: string): Promise<string> {

//   const data = clusterSettings(instanceSizeName, providerName, regionName, clusterName);
//   console.log(`Creating cluster ${clusterName} with config {instanceSize: "${instanceSizeName}", providerName: "${providerName}", regionName: "${regionName}"}`);

//   return await AtlasAuth.axios.request({
//     url: clusterUrl(),
//     method: 'POST',
//     data: data
//   }).then((response) => {
//     const data = response.data as ResponseData;
//     return data.connectionStrings.standardSrv;
//   }, (e) => {
//     throw new Error((e as AxiosError).response.data.detail);
//   });

// }

// export async function getClusters(projectId: string = getProjectId()): Promise<Cluster[]> {
//   console.log(`Getting clusters for project ${projectId}`);
//   return await AtlasAuth.axios.request({
//     url: clusterUrl(),
//     method: 'GET',
//   }).then(response => {
//     const results = response.data["results"] as Cluster[];
//     console.log(`Found ${results.length} clusters in project ${projectId}`);
//     return results;
//   }).catch(e => {
//     throw new Error((e as AxiosError).response.data.detail);
//   });
